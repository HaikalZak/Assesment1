...
fun DetailScreen(navController: NavHostController, id: String? = null){
    ...
    LaunchedEffect(true) {
        ...
        val data = viewModel.getBudar(id) ?: return@LaunchedEffect
        if (data != null) {
            judul = data.judul
        }
        if (data != null) {
...
fun getProvincesByIsland(island: String): List<String> {
    return when (island) {
        "Jawa Tengah" -> listOf("Kudus", "Semarang", "Blora", "Rembang", "Salatiga", "Demak", "Pati", "Jepara", "Klaten", "Tegal", "Purwokerto")
        else -> emptyList()
    }
}
